...
* atualização e remoção de pacotes somente de forma mediada por pac-base, não diretamento pelo pacman(discutido a seguir), e preferencialmente reiniciando a cada modificação. 
...
Base-manager --restore-root provê uma forma simples de tornar qualquer dos commits disponíveis em nova PacmanRoot a partir do boot seguinte; é uma operação destrutiva, e deve ser usada partir do boot de um dos commits. Só deve ser usado se seus commits forem gerados por commit-root; se usou outra ferramenta para gerar os commits, confie nela também para a restauração.
